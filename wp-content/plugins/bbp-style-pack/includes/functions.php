...
function bsp_display_buttons () {
	global $bsp_style_settings_buttons ;
	$topic_button = $subscribe_button = $profile_button = 0 ;
	...
	$total_buttons = $topic_button + $subscribe_button + $profile_button ;
	...
	$default_subscribe= 2 ;
	$default_profile = 3 ;
	//we need to sort out a default order if button are 2
	if ($total_buttons == 2) {
	if (!empty ($topic_button) ) $default_topic= 1 ;
	if (empty ($topic_button) && !empty ($subscribe_button) ) $default_subscribe= 1 ; 
	if (!empty ($topic_button) && !empty ($subscribe_button) ) $default_subscribe = 2 ;
	if (!empty ($topic_button) && empty ($subscribe_button) ) $default_profile = 2 ;
	}
	$order = array() ;
	$i=1 ;
...
function bsp_new_topic_button () {
	...
	echo '<a class="'.$class.'" href ="#bsptopic">'.$text.'</a>' ;
	}
}
...
function bsp_create_new_topica () {
	...
	if ( bbp_current_user_can_access_create_topic_form() && !bbp_is_forum_category() ) echo '<div class="bsp-new-topic">  <a href ="#bsptopic">'.$text.'</a></div>' ;
	}
...
function bsp_no_profile ($author_link, $r ) {
	...
	elseif ($bsp_profile['profile'] == 2) {
		//next linne needed for bbp_get_topic_author_link 
	...
	//then call the supress function to add it back
